...
mod verification {
    ...
    #[kani::proof]
    fn ping_route_has_exactly_one_allowed_method() {
        let method: [u8; 3] = kani::any();
        let method = String::from_utf8_lossy(&method);
        let response = response_for(&method, "/ping");

        if method == "GET" {
            assert!(response.status_code() == 200);
            assert!(response.allow().is_none());
        } else {
            assert!(response.status_code() == 405);
            assert!(response.allow() == Some("GET"));
        }
    }

    #[kani::proof]
    fn unknown_routes_are_never_successful() {
        let target: [u8; 8] = kani::any();
        let target = String::from_utf8_lossy(&target);
        kani::assume(!target.starts_with("/ping"));
        let response = response_for("GET", &target);
